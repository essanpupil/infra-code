...
data "terraform_remote_state" "erp_cluster" {
  backend = "s3"
  config  = { bucket = "platform-stg-terraform-state", key = "stg/singapore/eks/erp-cluster/terraform.tfstate", region = var.aws_region }
}
data "terraform_remote_state" "pos_cluster" {
  backend = "s3"
  config  = { bucket = "platform-stg-terraform-state", key = "stg/singapore/eks/pos-cluster/terraform.tfstate", region = var.aws_region }
}
module "eks" {
  source     = "git::https://github.com/essanpupil/infra-code.git//modules/aws/eks"
  name       = "platform-stg-support"
  ...
  tags = { Environment = "stg", ManagedBy = "terraform", Team = "platform" }
}
